...
contract StakingOracle {
    ...
    function reportPrice(uint256 price) public onlyNode {
        ...
        if (node.lastReportedBucket != 0 && node.lastReportedBucket < currentBucket) {
    ...
    function claimReward() public {
        ...
        node.claimedReportCount = node.reportCount; // set report count as now
    ...
    function recordBucketMedian(uint256 bucketNumber) public {
        ...
        //It should only allow this function to be called with past buckets,
        //otherwise revert with OnlyPastBucketsAllowed
        uint256 currentBucket = getCurrentBucketNumber();
        ...
        public
    {
        ...
        uint256 actualPenalty = node.stakedAmount > MISREPORT_PENALTY ? MISREPORT_PENALTY : node.stakedAmount;
        ...
        bool sent = oracleToken.transfer(msg.sender, reward);
    ...
    function exitNode(uint256 index) public onlyNode {
        ...
        uint256 effectiveState = getEffectiveStake(msg.sender);
        _removeNode(msg.sender, index);
        nodes[msg.sender].stakedAmount = 0;
        bool sent = oracleToken.transfer(msg.sender, effectiveState);
        if (!sent) revert TransferFailed();
        emit NodeExited(msg.sender, effectiveState);
    }
    ...
    function getLatestPrice() public view returns (uint256) {
        ...
        BlockBucket storage bucket = blockBuckets[latestBucket];
        if (bucket.medianPrice == 0) revert MedianNotRecorded();
    ...
    {
        BlockBucket storage bucket = blockBuckets[bucketNumber];
        uint256 bucketReportersLength = bucket.reporters.length;
        for (uint256 i = 0; i < bucketReportersLength; ++i) {
    ...
    function getEffectiveStake(address nodeAddress) public view returns (uint256) {
        ...
        if (getCurrentBucketNumber() == node.firstBucket) return node.stakedAmount;

        uint256 expectedReports = getCurrentBucketNumber() - node.firstBucket;
        uint256 actualReportCompleted = node.reportCount;
        // Don’t count a report in the current bucket as a “completed” report (it’s still in-flight)
        if (node.lastReportedBucket == getCurrentBucketNumber() && actualReportCompleted > 0) {
            actualReportCompleted -= 1;
        }
        if (actualReportCompleted >= expectedReports) return node.stakedAmount;
        uint256 missedBuckets = expectedReports - actualReportCompleted;
    ...
    function getOutlierNodes(uint256 bucketNumber) public view returns (address[] memory) {
        BlockBucket storage bucket = blockBuckets[bucketNumber];
        if (bucket.medianPrice == 0) revert MedianNotRecorded();
        address[] memory tempOutlierNodes = new address[](bucket.reporters.length);
        uint256 validCount = 0;
        for (uint256 i = 0; i < bucket.reporters.length; ++i) {
            if (
                _checkPriceDeviated(bucket.prices[i], bucket.medianPrice)
                    && !bucket.slashedOffenses[bucket.reporters[i]]
            ) {
                tempOutlierNodes[validCount] = bucket.reporters[i];
                validCount++;
            }
        }
        address[] memory outlierNodes = new address[](validCount);
        for (uint256 i = 0; i < validCount; ++i) {
            outlierNodes[i] = tempOutlierNodes[i];
        }
        return outlierNodes;
    }
    ...
    function _checkPriceDeviated(uint256 reportedPrice, uint256 medianPrice) internal pure returns (bool) {
        uint256 absAmount = reportedPrice > medianPrice ? reportedPrice - medianPrice : medianPrice - reportedPrice;
        uint256 deviationBPS = absAmount * 10000 / medianPrice;
        if (deviationBPS <= MAX_DEVIATION_BPS) return false;
        return true;
    }
}
