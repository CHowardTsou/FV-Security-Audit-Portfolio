...
contract OptimisticOracle {
        ...
        returns (uint256)
    {
        ...
        nextAssertionId++;
        uint256 setStartTime = startTime == 0 ? block.timestamp : startTime;
        uint256 setEndTime = endTime == 0 ? setStartTime + MINIMUM_ASSERTION_WINDOW : endTime;
        if (setStartTime < block.timestamp) revert InvalidTime();
        if (setEndTime - setStartTime < MINIMUM_ASSERTION_WINDOW) revert InvalidTime();
    ...
    function proposeOutcome(uint256 assertionId, bool outcome) external payable {
        EventAssertion storage assertedEvent = assertions[assertionId];
        if (assertedEvent.asserter == address(0)) revert AssertionNotFound();
        if (assertedEvent.proposer != address(0)) revert AssertionProposed();
        if (assertedEvent.startTime > block.timestamp || assertedEvent.endTime < block.timestamp) {
            revert InvalidTime();
        }
        if (assertedEvent.bond != msg.value) revert InvalidValue();
        assertedEvent.proposer = msg.sender;
        assertedEvent.proposedOutcome = outcome;
        assertedEvent.endTime = block.timestamp + DISPUTE_WINDOW;
    ...
    function disputeOutcome(uint256 assertionId) external payable {
        EventAssertion storage assertedEvent = assertions[assertionId];
        if (assertedEvent.proposer == address(0)) revert NotProposedAssertion();
        if (assertedEvent.disputer != address(0)) revert ProposalDisputed();
        if (block.timestamp > assertedEvent.endTime) revert InvalidTime();
        if (msg.value != assertedEvent.bond) revert InvalidValue();
        assertedEvent.disputer = msg.sender;
    ...
    function claimUndisputedReward(uint256 assertionId) external {
        ...
        if (assertedEvent.disputer != address(0)) revert ProposalDisputed();
        if (block.timestamp <= assertedEvent.endTime) revert InvalidTime();
        if (assertedEvent.claimed) revert AlreadyClaimed();
        assertedEvent.claimed = true;
        assertedEvent.winner = assertedEvent.proposer;
        assertedEvent.resolvedOutcome = assertedEvent.proposedOutcome;
        uint256 totalReward = assertedEvent.reward + assertedEvent.bond;
        (bool success,) = payable(assertedEvent.winner).call{value: totalReward}("");
        if (!success) revert TransferFailed();
        emit RewardClaimed(assertionId, assertedEvent.winner, totalReward);
    }
    ...
    function claimDisputedReward(uint256 assertionId) external {
        EventAssertion storage assertedEvent = assertions[assertionId];
        if (assertedEvent.proposer == address(0)) revert NotProposedAssertion();
        if (assertedEvent.disputer == address(0)) revert NotDisputedAssertion();
        if (assertedEvent.winner == address(0)) revert AwaitingDecider();
        if (assertedEvent.claimed) revert AlreadyClaimed();
        assertedEvent.claimed = true;
        (bool deciderSuccess,) = payable(decider).call{value: assertedEvent.bond}("");
        if (!deciderSuccess) revert TransferFailed();
        uint256 totalReward = assertedEvent.reward + assertedEvent.bond;
        (bool winnerSuccess,) = payable(assertedEvent.winner).call{value: totalReward}("");
    ...
    function claimRefund(uint256 assertionId) external {
        ...
        if (assertedEvent.claimed) revert AlreadyClaimed();
        assertedEvent.claimed = true;
        (bool success,) = payable(assertedEvent.asserter).call{value: assertedEvent.reward}("");
        if (!success) revert TransferFailed();
        emit RefundClaimed(assertionId, assertedEvent.asserter, assertedEvent.reward);
    }
    ...
    function settleAssertion(uint256 assertionId, bool resolvedOutcome) external onlyDecider {
        ...
        if (assertedEvent.proposer == address(0)) revert NotProposedAssertion();
        if (assertedEvent.disputer == address(0)) revert NotDisputedAssertion();
        if (assertedEvent.winner != address(0)) revert AlreadySettled();
        assertedEvent.resolvedOutcome = resolvedOutcome;
        assertedEvent.winner = assertedEvent.proposedOutcome == assertedEvent.resolvedOutcome
            ? assertedEvent.proposer
            : assertedEvent.disputer;
        emit AssertionSettled(assertionId, resolvedOutcome, assertedEvent.winner);
    }
    ...
    function getState(uint256 assertionId) external view returns (State) {
        EventAssertion storage assertedEvent = assertions[assertionId];
        if (assertedEvent.asserter == address(0)) return State.Invalid;
        if (assertedEvent.winner != address(0)) return State.Settled;
        if (assertedEvent.disputer != address(0)) return State.Disputed;
        if (assertedEvent.proposer == address(0)) {
            if (assertedEvent.endTime < block.timestamp) return State.Expired;
            else return State.Asserted;
        }
        if (assertedEvent.endTime < block.timestamp) return State.Settled;
    ...
    function getResolution(uint256 assertionId) external view returns (bool) {
        ...
        if (assertedEvent.asserter == address(0)) revert AssertionNotFound();
        if (assertedEvent.proposer == address(0)) revert NotProposedAssertion();
        if (assertedEvent.disputer == address(0)) {
            if (assertedEvent.endTime >= block.timestamp) revert InvalidTime();
        } else {
            if (assertedEvent.winner == address(0)) revert AwaitingDecider();
        }
        return assertedEvent.resolvedOutcome;
    }
}
